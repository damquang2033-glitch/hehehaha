...
import { useHostBookings, useUpdateBookingStatus } from "@/features/bookings/hooks/useBookings";
import { Booking, BookingStatus } from "@/types/booking";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Skeleton } from "@/components/ui/skeleton";
import { MapPin, CalendarDays, Users, CheckCircle, XCircle, Loader2 } from "lucide-react";
...
function HostBookingItem({ booking }: { booking: Booking }) {
  ...
  const handle = (status: BookingStatus) => {
    updateStatus(
      ...
      {
        onSuccess: () => toast.success(status === "CONFIRMED" ? "Đã xác nhận" : "Đã từ chối"),
      ...
      <div className="p-5 flex flex-col justify-between flex-1">
        <div className="space-y-2">
          <div className="flex items-start justify-between gap-2">
            <Link href={`/listings/${booking.listingId}`} className="font-bold text-slate-900 dark:text-white hover:text-orange-600 line-clamp-1">
        ...
        <div className="flex items-center justify-between mt-4">
          ...
          {booking.status === "PENDING" && (
            <div className="flex gap-2">
              <Button size="sm" onClick={() => handle("CONFIRMED")} disabled={isPending}
    ...
    <div className="container max-w-3xl py-10">
      ...
      {!isLoading && !data?.data.length && (
        <div className="text-center py-20 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800">
          <p className="text-slate-500">Chưa có booking nào cho các chỗ ở của bạn</p>
        </div>
      )}

      {!isLoading && data?.data.length ? (
        <div className="space-y-4">
          {data.data.map((booking) => <HostBookingItem key={booking.id} booking={booking} />)}
        </div>
      ) : null}
    </div>
  );
